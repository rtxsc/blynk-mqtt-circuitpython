...
# Modified Work Copyright (c) 2025 Robotronix to adapt with Blynk MQTT
# Mon 22 Sept 2025 changes are labelled with #22sepChanges
...
--------------------
Changes labelled with #modified-for-blynk
...
_default_sock = None
_fake_context = None
DEBUG_ALL = False #modified-for-blynk
DEBUG_INFO = False # must be explicitly enabled if not DEBUG_ALL #modified-for-blynk
...
class MQTT:  # noqa: PLR0904  # too-many-public-methods
    ...
    def __init__(  # noqa: PLR0915, PLR0913, Too many statements, Too many arguments
        ...
        *,
        ...
        socket_timeout: float = 1.0,    #22sepChanges - changed to float 
        connect_retries: int = 60,      #31DEC2025_CHANGES - to avoid frequent restart (New Year 2026 Eve MKE2)
        user_data=None,
    ) -> None:
        ...
        #modified-for-blynk
        if DEBUG_ALL:
            self.logger = adafruit_logging.getLogger(__name__)
            self.logger.setLevel(adafruit_logging.DEBUG)
        elif DEBUG_INFO:
            self.logger = adafruit_logging.getLogger(__name__)
            self.logger.setLevel(adafruit_logging.INFO)
        else:
    ...
    def connect(  # noqa: PLR0913, too many arguments in function definition
        ...
        last_exception = None
        backoff = True #modified-for-blynk
        for i in range(0, self._reconnect_attempts_max):
            if i >= 0: #modified-for-blynk
                if backoff:
            ...
            #31DEC2025_CHANGES
            self.logger.info(
                f"Attempting to connect to MQTT broker (attempt #{self._reconnect_attempt}/{self._reconnect_attempts_max})"
            )
        ...
        if self._reconnect_attempts_max > 1:
            exc_msg = f"[mqtt.connect] Repeated connect failures after retrying {self._reconnect_attempt} times"
        else:
            exc_msg = f"[mqtt.connect] Connect failure"
    ...
    def ping(self) -> list[int]:
        ...
        """
        self._connected()
        self.logger.info("Sending PINGREQ")
                    ...
                    for i in range(0, remaining_len - 2):
                        if rc[i] not in [0, 1, 2]:
                            ...
                            raise MMQTTException(
                                ...
                                f" Suggestion: IO_MQTT(mqtt_client, use_blynk=True)"
                            )
    ...
    def unsubscribe(  # noqa: PLR0912, Too many branches
        ...
        while True:
            ...
            if op is None:
                ...
                if op == MQTT_UNSUBACK:
                    rc = self._sock_exact_recv(3)
                    print(rc) # output is bytearray(b'\x03\x00\x02') #22sepChanges
                    assert rc[0] == 0x03 # 0x02 #22sepChanges
                    # [MQTT-3.32]
