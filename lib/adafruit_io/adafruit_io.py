...
def validate_feed_key(feed_key: str):
    ...
    if not bool(
        re.match(r"^[a-zA-Z0-9-#]+((\/|\.)[a-zA-Z0-9-#]+)?$", feed_key) #modified-for-blynk added '#' into regex
...
class IO_MQTT:
    ...
    def __init__(self, mqtt_client, use_blynk = False):
    ...
    def subscribe(
        ...
        if shared_user is not None and feed_key is not None:
            ...
            #modified-for-blynk
            if self._use_blynk:
                self._client.subscribe(f"{feed_key}") # Blynk subscribe format
            else:
                self._client.subscribe(f"{self._user}/f/{feed_key}") # AIO subscribe format
        else:
    ...
    def publish(
        ...
        if metadata is not None:
            ...
            #modified-for-blynk
            if self._use_blynk:
                self._client.publish(f"ds/{feed_key}", data)
            else:
    ...
    def get(self, feed_key: str):
        ...
        #modified-for-blynk
        if self._use_blynk:
            self._client.publish(f"get/ds", feed_key)
        else:
